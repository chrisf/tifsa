...
public class Report implements IReport{
     ...
     */
    private final String sql = "SELECT dbo.CustomerType.CustomerTypeName, dbo.Customer.CustomerFirst, dbo.Customer.CustomerLast, COUNT([Order].OrderID) AS TotalOrders, SUM([Order].OrderBalance) AS OutstandingBalance, dbo.CustomerStatus.CustomerStatusDescription AS [Status]\n" +
            "FROM Customer\n" +
            ...
            "WHERE CustomerStatus.CustomerStatusID = 3 AND Customer.Deleted = 0\n" +
            "GROUP BY CustomerType.CustomerTypeName,\n" +
            "   \t  [Order].CustomerID,\n" +
     ...
     */
    private final String name = "Customer Order Totals";
    ...
    public FXMLLoader getControls() throws IOException {
        return new FXMLLoader(getClass().getResource("/ReportControls/CustomerOrderTotals.fxml"));
    }
}
